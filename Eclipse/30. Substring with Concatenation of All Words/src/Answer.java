...
public class Answer {
    ...
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        for(int i=0; i<wl; i++){
        	int start=i, end=i, count = words.length;
        	HashMap<String, Integer> curDic = new HashMap<>(dic);
        	
            while(end+wl<=s.length()){
            	String cur = s.substring(end, end+wl);
            	...
            	if(dic.containsKey(cur)){
            		if(curDic.get(cur)>=1){
                		...
                		if(count==0){
                			res.add(start);
                			String sCur = s.substring(start, start+wl);
                            curDic.put(sCur, curDic.get(sCur)+1);
                			count++;
                			start+=wl;
                		}
            		}
                		...
                		String sCur = s.substring(start, start+wl);
                		while(!sCur.equals(cur) && start<=end){
                			curDic.put(sCur, curDic.get(sCur)+1);
                			count++;
                			start+=wl;
                			sCur = s.substring(start, start+wl);
                		}
                		start+=wl;
                		end+=wl;
            		}       		
